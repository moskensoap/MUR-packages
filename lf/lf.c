...
int main(int argc, char *argv[])
{
    ...
    // Replace "/bin/$(fileName)" with "/share/mur__lf/bin/$(fileName)"
    char binPath[PATH_MAX], realPath[PATH_MAX];
    sprintf(binPath, "/bin/%s", fileName);
    sprintf(realPath, "/share/mur__lf/bin/%s", fileName);
    ...
    // cp path to dataPath and replace /share/mur__lf/bin/$(fileName) with /share/mur__lf/data
    char dataPath[PATH_MAX];
    strcpy(dataPath, path);
    sprintf(binPath, "/share/mur__lf/bin/%s", fileName);
    sprintf(realPath, "/share/mur__lf/data");
    replacePtr = strstr(dataPath, binPath);
    if (replacePtr != NULL)
    {
        strncpy(replacePtr, realPath, strlen(realPath));
        replacePtr += strlen(realPath);
    ...
    // cp path to configPath and replace /share/mur__lf/bin/$(fileName) with /share/mur__lf/config
    char configPath[PATH_MAX];
    strcpy(configPath, path);
    sprintf(binPath, "/share/mur__lf/bin/%s", fileName);
    sprintf(realPath, "/share/mur__lf/config");
    replacePtr = strstr(configPath, binPath);
    if (replacePtr != NULL)
    {
        strncpy(replacePtr, realPath, strlen(realPath));
        replacePtr += strlen(realPath);
    ...
    setenv("ProgramData", dataPath, 1);
    setenv("LF_CONFIG_HOME", configPath, 1);

    setenv("OPENER", "less", 1);
    setenv("VISUAL", "vim", 1);
    setenv("EDITOR", "vim", 1);
    setenv("PAGER", "less", 1);
