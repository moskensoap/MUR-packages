...
int main(int argc, char *argv[])
{
    ...
    // build a clang32list 'findPath *clang32Path -maxdepth 3 -exec pacmanPath -Qo {} + 2>&1 | grepPath "error: No package owns"'
    char clang32list[PATH_MAX * 4 + 63];
    sprintf(clang32list, "%s %s -maxdepth 3 -exec %s -Qo {} + 2>&1 | %s \"error: No package owns\"", findPath, clang32Path, pacmanPath, grepPath);
    // build a clang64list 'findPath *clang64Path -maxdepth 3 -exec pacmanPath -Qo {} + 2>&1 | grepPath "error: No package owns"'
    char clang64list[PATH_MAX * 4 + 63];
    sprintf(clang64list, "%s %s -maxdepth 3 -exec %s -Qo {} + 2>&1 | %s \"error: No package owns\"", findPath, clang64Path, pacmanPath, grepPath);
    // build a clangarm64list 'findPath *clangarm64Path -maxdepth 3 -exec pacmanPath -Qo {} + 2>&1 | grepPath "error: No package owns"'
    char clangarm64list[PATH_MAX * 4 + 63];
    sprintf(clangarm64list, "%s %s -maxdepth 3 -exec %s -Qo {} + 2>&1 | %s \"error: No package owns\"", findPath, clangarm64Path, pacmanPath, grepPath);
    // build a mingw32list 'findPath *mingw32Path -maxdepth 3 -exec pacmanPath -Qo {} + 2>&1 | grepPath "error: No package owns"'
    char mingw32list[PATH_MAX * 4 + 63];
    sprintf(mingw32list, "%s %s -maxdepth 3 -exec %s -Qo {} + 2>&1 | %s \"error: No package owns\"", findPath, mingw32Path, pacmanPath, grepPath);
    // build a mingw64list 'findPath *mingw64Path -maxdepth 3 -exec pacmanPath -Qo {} + 2>&1 | grepPath "error: No package owns"'
    char mingw64list[PATH_MAX * 4 + 63];
    sprintf(mingw64list, "%s %s -maxdepth 3 -exec %s -Qo {} + 2>&1 | %s \"error: No package owns\"", findPath, mingw64Path, pacmanPath, grepPath);
    // build a ucrt64list 'findPath *ucrt64Path -maxdepth 3 -exec pacmanPath -Qo {} + 2>&1 | grepPath "error: No package owns"'
    char ucrt64list[PATH_MAX * 4 + 63];
    sprintf(ucrt64list, "%s %s -maxdepth 3 -exec %s -Qo {} + 2>&1 | %s \"error: No package owns\"", findPath, ucrt64Path, pacmanPath, grepPath);
    // build a usrlist 'findPath *usrPath -maxdepth 3 -exec pacmanPath -Qo {} + 2>&1 | grepPath "error: No package owns"'
    char usrlist[PATH_MAX * 4 + 63];
    sprintf(usrlist, "%s %s -maxdepth 3 -exec %s -Qo {} + 2>&1 | %s \"error: No package owns\"", findPath, usrPath, pacmanPath, grepPath);
    // build a optlist 'findPath *optPath -maxdepth 3 -exec pacmanPath -Qo {} + 2>&1 | grepPath "error: No package owns"'
    char optlist[PATH_MAX * 4 + 63];
    sprintf(optlist, "%s %s -maxdepth 3 -exec %s -Qo {} + 2>&1 | %s \"error: No package owns\"", findPath, optPath, pacmanPath, grepPath);
    // build a tmplist 'findPath *tmpPath -maxdepth 3 -exec pacmanPath -Qo {} + 2>&1 | grepPath "error: No package owns"'
